...
def get_guess(max_value):
    user = input("guess a random integer between 1 and the max value.")
    if user.isdigit():
        user = int(user)
    else:
        return -1
    computer = get_random_int(1, max_value)
...
def play_game():
    ...
    print(f"You guessed {percentage:.0f}% of the random numbers correctly.")
